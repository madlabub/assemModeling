function [AdjMat, prtSzRatio] = getAdjNSz(clsObjPrt, thrshld, nP, avgNPrt)
...
AdjMat = cell(nCls,1);
objSz = cell(nCls,1);
prtSzRatio = cell(nCls,1);
for c = 1 : nCls
    nPrt = clsObjPrt{c,1}{end,1}(end,1);
    AdjMat{c,1} = zeros(nPrt, nPrt);
    for o = 1 : length(clsObjPrt{c,1});
        objFile = [dir,'\Database\Class',num2str(c),'\O',num2str(o),'.obj'];
        P = UniformSampling(objFile, nP * avgNPrt(c,1)); % sampling points on the object
        ...
        for n1 = 1 : length(clsObjPrt{c,1}{o,1})
            ...
            [~, pcaP] = princomp(P1); % aligning with principal components
            mx = max(pcaP); % maximum coordinate along each principal component
            mn = min(pcaP); % minimum coordinate along each principal component
            prtSzRatio{c,1}{o,1}(n1,1) = prod(mx-mn)/objSz{c,1}(o,1); % length of the diagonal of bounding box
            for n2 = n1+1 : length(clsObjPrt{c,1}{o,1})
                ...
                if mnDist < thrshld*S % setting 1 for part pairs with distance less than some threshold
